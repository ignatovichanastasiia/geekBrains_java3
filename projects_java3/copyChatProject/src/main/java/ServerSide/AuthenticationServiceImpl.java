...
//            statement.execute("insert into users (login,password,nick) values ('C','C','C');");
    ...
    public boolean registration(String login, String password, String nick) {
        try (PreparedStatement psn = new PreparedStatement()){
            psn = connection.prepareStatement("select from users nick where login = ?");
        ...
        try(PreparedStatement psq = new PreparedStatement()) {
            if (resSet == null) {
